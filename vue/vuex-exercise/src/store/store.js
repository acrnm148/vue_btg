...
export default new Vuex.Store({
    ...
    // 위의 state를 바꾸는 유일한 방법. 저 위의 값을 바꾸려면 mutation으로 바꿔야 한다
    mutations: {
        ...
        SET_BOARD(state, payload) { //state: 여기꺼, payload: object 형태로 가져온것
    ...
    // 필요한 기능/동작을 수행함, 일반적으로 수행 결과를 state에 반영하기 위한 메소드 집합, state에 반영할 때는 mutations를 이용
    // BackEnd 요청 / 결과 처리
    // BackEnd 요청 / 결과 처리 코드는 component에 따라서 Component 자체에 있을 수도 있고, vuex store에 코드가 있을 수도 있다
    // 비동기 처리가 많다.
    actions: {
        setMessage(context, message) { // setMessage가 비동기(async,await), 동기
    ...
    // 그대로 또는 state의 데이터의 변형을 처리한후, 결과를 return이 있는 메소드들
    getters: {
